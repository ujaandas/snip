...
void LspClient::didOpen(const QString &uri, const QString &languageId,
  ...
  auto params = QJsonObject{
      {"textDocument", JsonRpc::textDocumentItem(uri, languageId, text)}};
...
void LspClient::requestSemanticTokens(const QString &uri) {
  ...
  auto params = QJsonObject{{"textDocument", JsonRpc::textDocument(uri)}};
  sendMessage(
      JsonRpc::request("textDocument/semanticTokens/full", params, requestId_));
}
...
void LspClient::onReadyRead() {
  ...
  while (true) {
    ...
    if (expectedLength_ < 0 || readBuffer_.size() < expectedLength_)
      break;
...
void LspClient::handleSemanticTokensResponse(int requestId,
